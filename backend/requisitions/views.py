...
# Importa los permisos desde users.permissions
...
class RequisicionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.profile.roles.filter(name='COMPRAS').exists():
    ...
    # --- ACCIONES PARA APROBACIÓN / RECHAZO ---
    ...
    def all_requisitions(self, request):
        ...
        """
        # IMPORTANTE: Volver a poner esta línea para obtener TODAS las requisiciones
        queryset = Requisicion.objects.all().order_by('-fecha_creacion', '-id')

        # El resto de la paginación puede quedar igual
